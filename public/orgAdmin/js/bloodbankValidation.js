$(function() {
    ...
    var formErrors = ['.name', '.email', '.address', '.imageprofile', '.imagelogo', '.selected-phone-code', '.phone-number', '.url', '.url-facebook', '.url-twitter', '.url-instagram', '.url-youtube', '.url-GoogleMap', '.selected-governorate', '.selected-city', '.about_us'];
    var regExpValidation = {
        "nameMinMaxLength": /^.{4,100}$/,
        "name": /^[a-zA-Z][\w_\-\s\d]{3,99}$/,
        "address": /^[\d a-zA-Z][\d a-zA-Z\s\.\-\_\,]{9,}$/,
        'about_us': /^[a-zA-Z0-9 .\s\.\/\_\-\,\.\!\#\$\%\\&\'\*\+\/\=\?\^\_\`\{\|\}\~\-\t\r\n]{10,5000}$/,
    ...
    var formErrorsMessage = {
        ...
        "describtion": "Describtion may not be empty!!! And minimum 10 Chracters",
        "Password": "",
        "image": "",
    ...
    $('.imagelogo').change(function() {
        ...
        if (formErrorsMessage.image == "") { /* If there is no Error */
            $(this).removeClass('is-invalid').addClass('is-valid').parent().parent().find('.custom-alert').fadeOut(250);
            // console.log("image Valid");
            if (formErrors.includes(".imagelogo"))
                removeSpecificElement(formErrors, '.imagelogo');
        } else {
            $(this).removeClass('is-valid').addClass('is-invalid').parent().parent().find('.custom-alert').fadeIn(500).html(formErrorsMessage.image);
            if (!formErrors.includes(".imagelogo"))
                formErrors.push('.imagelogo');
        }
    ...
    $('.ValidationForm').submit(function(e) {
        if (
            ...
            $.inArray('.imagelogo', formErrors) != -1 ||
            ...
            $('.imageprofile, .imagelogo, .selected-governorate, .selected-city').change(); // To Show Error for Image has event is blur and have Error 
        }
